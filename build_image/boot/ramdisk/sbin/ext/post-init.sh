...
if [ "`grep "kernel.turbo=true" $PROP`" != "" ]; then
	...
	echo "Kernel Turbo-mode enabled." >> $LOG;
fi;
...
OPEN_RW()
{
	...
	echo "System mounted." >> $LOG;
}
OPEN_RW;

###############################################################################
# Start SuperSU deamon

/system/xbin/daemonsu --auto-daemon &;
echo "SuperSU deamon started successful." >> $LOG;
...
if [ "`grep "kernel.interactive=performance" $PROP`" != "" ]; then
	...
	echo "Interactive Performance tweaks applied." >> $LOG;
	...
	echo "15000"	> /sys/devices/system/cpu/cpu4/cpufreq/interactive/timer_rate;
	echo "Interactive Battery tweaks applied." >> $LOG;
fi;
...
if [ "`grep "kernel.vm=tuned" $PROP`" != "" ]; then
	...
	echo "Virtual Machine was tuned." >> $LOG;
fi;
